...
class KnapsackCrypto:
    ...
    def generate_key_pair(self):
        w = [randint(1, 100) for _ in range(self.n)]  # Веса элементов рюкзака
        q = randint(sum(w) + 1, sum(w) * 2)  # Случайное число q

        # Генерация открытого ключа
        public_key = [q * wi % 256 for wi in w]
    ...
    def encrypt(self, message):
        cipher_text = sum(ord(char) * self.public_key[i] for i, char in enumerate(message))
        return cipher_text

    def decrypt(self, cipher_text):
        w, q = self.private_key
        s = (cipher_text * pow(q, -1, 256)) % 256
        decrypted_message = ''.join(chr(s // wi) for wi in reversed(w))
...
if __name__ == "__main__":
    ...
    print(f"Открытый ключ: {crypto.public_key}")
    print(f"Зашифрованное сообщение: {encrypted}")
    print(f"Расшифрованное сообщение: {decrypted}")

    # Запись закрытого ключа в файл
    with open("private_key.txt", "w") as f:
        f.write(f"q: {crypto.private_key[1]}\n")
        f.write(f"w: {', '.join(map(str, crypto.private_key[0]))}\n")
